  ...
  describe "GET /autocomplete" do
    context "when user is gamemaster" do
      ...
      it "returns campaign attributes" do
        ...
        expect(body["campaigns"][0].keys).to eq(["id", "name", "entity_class"])
      end
      ...
      it "gets only active campaigns when show_hidden is false" do
        get "/api/v2/campaigns", params: { autocomplete: true, show_hidden: "false" }, headers: @gamemaster_headers
        expect(response).to have_http_status(:success)
        body = JSON.parse(response.body)
        expect(body["campaigns"].map { |c| c["name"] }).to eq(["Quest", "Adventure"])
      ...
      it "gets all campaigns when show_hidden is true" do
        get "/api/v2/campaigns", params: { autocomplete: true, show_hidden: "true" }, headers: @gamemaster_headers
        expect(response).to have_http_status(:success)
        body = JSON.parse(response.body)
      ...
      it "returns empty array when ids is explicitly empty" do
        get "/api/v2/campaigns", params: { autocomplete: true, ids: "" }, headers: @gamemaster_headers
        expect(response).to have_http_status(:success)
        body = JSON.parse(response.body)

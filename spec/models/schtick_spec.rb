...
RSpec.describe Schtick, type: :model do
  ...
  describe "validations" do
    it "requires a name" do
      schtick = Schtick.new
      schtick.valid?
      expect(schtick.errors[:title]).to include("can't be blank")
    end
  ...
  describe "associations" do
    it "belongs to a campaign" do
      schtick = Schtick.new(title: "Schtick", campaign: action_movie)
      expect(schtick.campaign).to eq(action_movie)
    end

    it "has a prerequisite" do
      schtick = Schtick.new(title: "Schtick", campaign: action_movie)
      schtick.prerequisite = Schtick.new(title: "Prerequisite", campaign: action_movie)
      expect(schtick.prerequisite.title).to eq("Prerequisite")
    end
  ...
  describe ".for_archetype" do
    let!(:schtick) { Schtick.create!(title: "Schtick", campaign: action_movie, archetypes: ["Exorcist Monk", "Everyday Hero"]) }

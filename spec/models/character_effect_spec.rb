...
RSpec.describe CharacterEffect, type: :model do
  ...
  it "must belong to a fight" do
    @character_effect = CharacterEffect.new(name: "Bonus", character_id: @brick.id)
    expect(@character_effect).not_to be_valid
    expect(@character_effect.errors[:shot]).to eq(["can't be blank"])
  end

  it "must belong to either a character or a vehicle" do
    @character_effect = @fight.character_effects.new(name: "Bonus")
    expect(@character_effect).not_to be_valid
  ...
  it "must have action_value if change is set" do
    @character_effect = @fight.character_effects.new(name: "Bonus", change: "+1", character_id: @brick.id)
    expect(@character_effect).not_to be_valid
  ...
  it "must have change if action_value is set" do
    @character_effect = @fight.character_effects.new(name: "Bonus", action_value: "MainAttack", character_id: @brick.id)
    expect(@character_effect).not_to be_valid
    expect(@character_effect.errors[:change]).to eq(["must be present if action_value is set"])
  end

  it "must be a valid action_value" do
    @character_effect = @fight.character_effects.new(name: "Bonus", action_value: "Thing", change: "+1", character_id: @brick.id)
    expect(@character_effect).not_to be_valid
    expect(@character_effect.errors[:action_value]).to eq(["must be a valid key"])
  end

  it "must be a valid action_value" do
    @character_effect = @fight.character_effects.new(name: "Bonus", action_value: "Thing", change: "+1", vehicle_id: @truck.id)
    expect(@character_effect).not_to be_valid
    expect(@character_effect.errors[:action_value]).to eq(["must be a valid key"])
  end
end
